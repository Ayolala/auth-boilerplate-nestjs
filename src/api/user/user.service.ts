...
export class UserService {
  ...
  async update(id: string, user: Partial<User>) {
    ...
    const existingUser = await this.userRepository.findOne({
      select: ['id', 'phone_otp_verified', 'image', 'no_of_children', 'marital_status', 
        'bvn_otp_verified', 'date_of_birth', 'document_state', 'tier'],
      where: [ { id } ]
    })
    const tier = getTier({ ...existingUser, ...user})
    const result = await this.userRepository.update(id, { ...user, tier });
